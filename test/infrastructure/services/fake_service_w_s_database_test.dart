...
void main() {
  ...
  test('read devuelve error si no hay ledger', () async {
    final Either<ErrorItem, Map<String, dynamic>> result = await service.read(
      'path',
    );
    expect(result.isLeft, true);
    expect(
      result.when((ErrorItem err) => err.code, (Map<String, dynamic> ok) => ''),
      'NOT_FOUND',
    );
  });

  test('write válido actualiza el ledger y retorna Right', () async {
    final Either<ErrorItem, void> result = await service.write(
      path,
      sampleLedger,
    );
  ...
  test('reset limpia el ledger y emite error en stream', () async {
    ...
    service.onValue(path).listen((
      ...
      event.when((ErrorItem error) => errores.add(error), (_) {});
    });
